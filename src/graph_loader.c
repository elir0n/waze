                          ...
                          const char* edges_path)
{
    ...
    while (read_line(fn, line, sizeof(line))) {
        ...
        int node_id;
        double lat, lon;
        if (parse_int_strict(node_id_s, &node_id) != 0 ||
        ...
        graph_set_node_coordinates(g, node_id, lat, lon);
        loaded_nodes++;
    }
    ...
    while (read_line(fe, line, sizeof(line))) {
        ...
        int edge_id, from, to, lanes, is_oneway;
        double len, speed;
        if (parse_int_strict(edge_id_s, &edge_id) != 0 ||
            parse_int_strict(from_s, &from) != 0 ||
            parse_int_strict(to_s, &to) != 0 ||
